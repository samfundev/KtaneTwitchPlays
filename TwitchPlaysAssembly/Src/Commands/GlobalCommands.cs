...
static class GlobalCommands
{
	...
	public static void Help(string user, bool isWhisper)
	{
		...
		IRCConnection.SendMessage(string.Format("!{0} manual [link to module {0}'s manual] | Go to {1} to get the vanilla manual for KTaNE", randomCodes[0], UrlHelper.Instance.VanillaManual), user, !isWhisper);
	...
	public static void ReadModuleInformation([Group(1)] string command, [Group(2)] string parameter, string user, bool isWhisper)
	{
		...
		switch (modules.Count)
		{
			...
			default:
				...
				IRCConnection.SendMessage($"Sorry, there is more than one module matching your search term. They are: {modules.Select(x => $"“{modules[0].moduleDisplayName}” ({modules[0].moduleID})").Join(", ")}", user, !isWhisper);
				break;
		}
	}
