...
public static class ComponentSolverFactory
{
	...
	static ComponentSolverFactory()
	{
		...
		ModComponentSolverInformation["algebra"] = new ModuleInformation { moduleScore = 9, helpText = "Type '!{0} submit <value>' OR '!{0} <value> submit'. Type '!{0} clear' to clear the screen." };
		...
		ModComponentSolverInformation["Backgrounds"] = new ModuleInformation { moduleScore = 5, helpText = "Use \'!{0} press 5\' to press the \"Push Me\" button 5 times, Use \'!{0} Submit\' to Submit the current answer. Note: Only takes numbers 1-9" };
		ModComponentSolverInformation["BattleshipModule"] = new ModuleInformation { moduleScore = 8, helpText = "Scan the safe spots with !{0} scan A2 B3 E5. Mark the spots as water with !{0} miss A1 A3 B4. Mark the spots as ships with !{0} hit E3 E4. Fill in the rows with !{0} row 3 4. Fill in columns with !{0} col B D" };
		ModComponentSolverInformation["BigCircle"] = new ModuleInformation { moduleScore = 12, DoesTheRightThing = true, moduleDisplayName = "Big Circle" };
		...
		ModComponentSolverInformation["BlindMaze"] = new ModuleInformation { moduleScore = 6, helpText = "Use !{0} nwse or !{0} uldr to move North West South East. Use !{0} reset to reset back to the start." };
		...
		ModComponentSolverInformation["BrailleModule"] = new ModuleInformation { moduleScore = 11, helpText = "Use !{0} cycle to show the Braille letters. Use !{0} press 1 to press the first letter, etc." };
		...
		ModComponentSolverInformation["burglarAlarm"] = new ModuleInformation { moduleScore = 8, helpText = "Activate the module using !{0} activate , Submit the correct answer using !{0} submit ######## ." };
		...
		ModComponentSolverInformation["CoordinatesModule"] = new ModuleInformation { moduleScore = 16, helpText = "Cycle the options with !{0} cycle. Submit your answer with !{0} submit <3,2>. Partial answers are acceptable. To do chinese numbers, its !{0} submit chinese 12.", DoesTheRightThing = false };
		...
		ModComponentSolverInformation["FlagsModule"] = new ModuleInformation { moduleScore = 10, helpText = "Cycle the flags with !{0} cycle. Move using !{0} left/right. Set to index 3 with !{0} set 3. Set to Canada with !{0} set Canada. Submit the current flag with !{0} submit. Submit Canada with !{0} submit Canada." };
		...
		ModComponentSolverInformation["HumanResourcesModule"] = new ModuleInformation { moduleScore = 9, helpText = "“!{0} cycle” to see all the names and traits. “!{0} fire X” to fire someone; “!{0} hire X” to hire someone." };
		ModComponentSolverInformation["hunting"] = new ModuleInformation { moduleScore = 10, helpText = "Press a button with !17 press #." };
		...
		ModComponentSolverInformation["identityParade"] = new ModuleInformation { moduleScore = 6, helpText = "Cycle with “!{0} cycle hair/build/attire/suspect/all”. Use “!{0} convict H B A S” to submit a solution, where H B A S must be a valid hair, build, attire, and suspect (in that order). Abbreviations must be the start of the word (e.g. use “tank” instead of “tank top”; “shirt” won’t work for “T-shirt”). Invalid submissions incur a penalty." };
		ModComponentSolverInformation["jukebox"] = new ModuleInformation { moduleScore = 3, helpText = "Type '!{0} press 321' (1 = top; 2 = middle; 3 = bottom.)" };
		...
		ModComponentSolverInformation["ledGrid"] = new ModuleInformation { moduleScore = 4, helpText = "Type '!{0} press abcd'" };
		...
		ModComponentSolverInformation["maintenance"] = new ModuleInformation { moduleScore = 10, helpText = "Type '!{0} Brake fluid change, Headlight bulb, Wash' to perform repairs in that order. Abbreviations are allowed, but note that ambiugous ones like “Windscreen” may trigger either “Windscreen replacement” or “Windscreen chip”." };
		ModComponentSolverInformation["Mastermind Simple"] = new ModuleInformation { moduleScore = 12, manualCode = "Mastermind", helpText = "Query the currently set colors with !{0} query. Query specific colors with !{0} query r b g y m. Submit the currently set colors with !{0} submit.  Submit specific colors with !{0} submit r b g y m. [Valid colors are R, G, B, M, Y, W]", DoesTheRightThing = true };
		ModComponentSolverInformation["Mastermind Cruel"] = new ModuleInformation { moduleScore = 15, helpText = "Query the currently set colors with !{ 0 } query.Query specific colors with !{ 0 } query r b g y m.Submit the currently set colors with !{ 0 } submit.Submit specific colors with !{ 0 } submit r b g y m. [Valid colors are R, G, B, M, Y, W]", DoesTheRightThing = true };
		ModComponentSolverInformation["MinesweeperModule"] = new ModuleInformation { moduleScore = 20, DoesTheRightThing = true };
		ModComponentSolverInformation["modernCypher"] = new ModuleInformation { moduleScore = 7, helpText = "Submit the decrypted word with !{0} submit printer." };
		...
		ModComponentSolverInformation["monsplodeCards"] = new ModuleInformation { moduleScore = 6, helpText = "View all cards with !{0} cycle. Select the card to trade with !{0} left or !{0} right. Trade selected card with !{0} trade. Keep your cards with !{0} keep.", DoesTheRightThing = true };
		...
		ModComponentSolverInformation["mortalKombat"] = new ModuleInformation { moduleScore = 7, helpText = "Type '!{0}' followed by either u/up, d/down, l/left, r/right, a, b or c (e.g. !{0} up down a b)" };
		...
		ModComponentSolverInformation["spwizPerspectivePegs"] = new ModuleInformation { moduleScore = 5, helpText = "Look for the peg with specific color using !{0} rotate pegs. Read off the color sequence with !{0} rotate br. Look at the peg lines with !{0} rotate. Look at a specific line with !{0} look bl. Press the pegs with !{0} press bl t br. | Positions in clockwise order are T, TR, BR, BL, TL.", DoesTheRightThing = true };
		ModComponentSolverInformation["PerplexingWiresModule"] = new ModuleInformation { moduleScore = 9, helpText = "Cut the wires with !{0} cut 2 3 1. The wires are numbered according to their connection on the bottom." };
		...
		ModComponentSolverInformation["poetry"] = new ModuleInformation { moduleScore = 4, helpText = "Submit your answer with !{0} press bunny, or !{0} bunny." };
		ModComponentSolverInformation["PointOfOrderModule"] = new ModuleInformation { moduleScore = 7, DoesTheRightThing = true };
		ModComponentSolverInformation["poker"] = new ModuleInformation { moduleScore = 8, helpText = "Type '!{0} 'press' followed by either 'fold', 'check', 'min', 'max', 'allin', 'bluff', 'truth', 'card1', 'card2', 'card3' or 'card4' (e.g. !{0} press fold)" };
		ModComponentSolverInformation["PolyhedralMazeModule"] = new ModuleInformation { moduleScore = 12, helpText = "Express your move as a sequence of numbers, e.g. “!{0} 1 2 3 4”. The first number in each command is a clockface direction (1–12) and selects the arrow closest to that direction. All subsequent numbers within the same command select an edge counting clockwise from the edge that was traversed last. For example, 1 is an immediate left-turn. In a five-sided face, 4 is an immediate right-turn. Use “!{0} reset” to reset the module." };
		ModComponentSolverInformation["PressX"] = new ModuleInformation { moduleScore = 5 };
		ModComponentSolverInformation["radiator"] = new ModuleInformation { moduleScore = 6, helpText = "Submit the temperature and water together with !{0} submit 12 34. Reset with !{0} reset." };
		...
		ModComponentSolverInformation["SetModule"] = new ModuleInformation { moduleScore = 6, helpText = "Use “!{0} press a1 a2” to press any number of buttons in that order, using a–c for columns and 1–3 for rows, or “!{0} press tm br” (top middle, bottom right)." };
		...
		ModComponentSolverInformation["sonic"] = new ModuleInformation { moduleScore = 9, helpText = "Type '!{0} press' followed by either TVon, TVoff, RBt, In, EL, Rg or start (e.g. !{0} press TVon; !{0} press RBt;)" };
		...
		ModComponentSolverInformation["symbolicCoordinates"] = new ModuleInformation { moduleScore = 6, helpText = "Type '!{0} submit [letter][digit]' (e.g. !{0} submit Z3)" };
		ModComponentSolverInformation["SymbolCycleModule"] = new ModuleInformation { moduleScore = 12, helpText = "Flip the switch by doing !{0} flip. The module will cycle through the screens automatically, but you can do it again using !{0} cycle. Click a screen a certain number of times by doing !{0} click left 3, which will click the left screen 3 times." };
		...
		ModComponentSolverInformation["timezone"] = new ModuleInformation { moduleScore = 5, helpText = "Use !{0} submit HHMM (HH = hours and MM = minutes) to submit your conversion result." };
		ModComponentSolverInformation["visual_impairment"] = new ModuleInformation { moduleScore = 5, helpText = "Use “!{0} a4 b3 d2” to press the squares. Letters are columns and numbers are rows." };
		ModComponentSolverInformation["wastemanagement"] = new ModuleInformation { moduleScore = 15, helpText = "Allocate the number 66 to waste with !{0} LXVIW. Change the W to an R for recycling. Reset the module with !{0} Reset. Submit the answer with !{0} Submit." };
