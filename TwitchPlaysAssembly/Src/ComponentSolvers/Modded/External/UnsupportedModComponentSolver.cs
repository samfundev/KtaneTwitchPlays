...
public class UnsupportedModComponentSolver : ComponentSolver
{
		...
		: base(module, componentSolverFields == null || componentSolverFields.HookUpEvents)
	{
		...
		ModInfo = new ModuleInformation { scoreString = "0", helpText = $"Solve this {(_bombModule != null ? "module" : "needy")} with !{{0}} solve", moduleDisplayName = $"Unsupported Twitchplays Module  ({module.BombComponent.GetModuleDisplayName()})", moduleID = "UnsupportedTwitchPlaysModule" };
